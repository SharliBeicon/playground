...
pub fn build(b: *std.Build) !void {
    ...
    if (target.query.os_tag == .emscripten) {
        const exe_lib = try emcc.compileForEmscripten(b, "playground", "src/main.zig", target, optimize);
        ...
        const run_option = b.step("run", "Run playground");
        run_option.dependOn(&run_step.step);
        return;
    }

    const exe = b.addExecutable(.{ .name = "playground", .root_source_file = b.path("src/main.zig"), .optimize = optimize, .target = target });

    exe.linkLibrary(raylib_artifact);
    exe.root_module.addImport("raylib", raylib);

    const run_cmd = b.addRunArtifact(exe);
    const run_step = b.step("run", "Run playground");
